...
class insieme{
			...
			public:
				...
				int findMin();
				void spostaAvanti(int i);
				void inserisci(type elemento,int i);
...
template<class T>
void insieme<T>::inserisci(type elemento, int i){
	this->elementi[i]=elemento;
}



template<class T>
typename insieme<T>::type insieme<T>::getelem(int i){
...
template<class T>
void insieme<T>::cancelladiz(int i){
...
template<class T>
void insieme<T>::spostaAvanti(int i){
	
	lunghezza++;
	for(int j=lunghezza-1; j>i-1; j--){
